...
static int __init ir_al_huerto_init(void)
{
  ...
  printk(KERN_DEBUG "El pages arriba ben descansat a l'hort, preparat per una dura jornada de feina\n");
...
static void __exit salir_del_huerto(void)
{
  ...
  printk(KERN_DEBUG "El pages ha arribat a casa sa i estalvi, ningu l'hi ha robat els melons\n");
...
activar_sys_call(int quina){
  int i;
  if(quina <-1 || quina > N_CRIDES_A_MONITORITZAR) return -EINVAL;
  else if(quina!=-1) activar_monitoritzatcio(quina);
  else
...
desactivar_sys_call(int quina){
  int i;
  if(quina <-1 || quina > N_CRIDES_A_MONITORITZAR) return -EINVAL;
  else if(quina!=-1) desactivar_monitoritzacio(quina);
  else
    for(i=0; i < N_CRIDES_A_MONITORITZAR; i++)
