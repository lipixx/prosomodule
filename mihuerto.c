...
imprimir_estadistiques (int pid)
{
  ...
  if (task_pid >= 0)
    {
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[OPEN]);
      ...
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[CLOSE]);
      printk ("Num CLOSES         : %i\n", pidstats->num_entrades);
      ...
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[WRITER]);
      printk ("Num WRITES         : %i\n", pidstats->num_entrades);
      ...
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[CLONE]);
      printk ("Num CLONES         : %i\n", pidstats->num_entrades);
      ...
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[LSEEK]);
      printk ("Num LSEEKS         : %i\n", pidstats->num_entrades);
      ...
      pidstats =
	&(((struct th_info_est *) task_pid->
	   thread_info)->estadistiques[N_CRIDES_A_MONITORITZAR]);
...
obtenir_estadistiques (int pid, int crida, struct pid_stats *stats)
{
  ...
  printk (KERN_EMERG "\n1=%i\n2=%i\n3=%i\n4=%lld\n\n", stats->num_entrades,
	  stats->num_sortides_ok, stats->num_sortides_error,
	  stats->durada_total);
  return 0;
}
